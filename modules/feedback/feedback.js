...
Drupal.behaviors.feedbackForm = function (context) {
  $('#block-feedback-form:not(.feedback-processed)', context).addClass('feedback-processed').each(function () {
    ...
    $block.find('span.feedback-link')
      .prepend('<span id="feedback-form-toggle"><div class="test" style="margin-top:-25px; "></div> </span> ')
	  .css('cursor', 'pointer')
      .toggle(function () {
...
Drupal.feedbackFormToggle = function ($block, enable) {
  ...
  if (enable) {
    $('#feedback-form-toggle', $block).html('<div class="test" style="margin-top:-25px;"></div> ');
	$("#block-feedback-form").attr('style','width:0px !important; ');
	$('#block-feedback-form').animate({'right':'0px'}, {queue:false,duration:500});
	 $('#block-feedback-form .content').animate({'width':'0px'}, {queue:false,duration:500});
    $('#block-feedback-form .content').animate({'width':'0px'}, {queue:false,duration:500});
  }
  else {
	  $("#block-feedback-form").attr('style','width:220px !important; ');
    $('#feedback-form-toggle', $block).html('<div class="test" style="margin-left:-41px; margin-top:-25px; "></div>');
	$('#block-feedback-form').animate({'right':'0px'}, {queue:false,duration:500});
    $('#block-feedback-form .content').animate({'width':'220px'}, {queue:false,duration:500});
  }
};
...
$('#feedback-error-message').remove();
 if (!$(':input[name="message"]', $form).val()) {
	...
	$('#edit-message-wrapper').append('<div id="feedback-error-message" class="form-required">Please enter a message.</div>');
